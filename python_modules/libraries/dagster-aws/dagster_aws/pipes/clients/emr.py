...
class PipesEMRClient(PipesClient, TreatAsResourceParam):
    ...
    Args:
        ...
        s3_application_logs_prefix (Optional[str]): The prefix to use when looking for application logs in S3. Defaults to `containers`.
    """
    ...
    def __init__(
        ...
        s3_application_logs_prefix: Optional[str] = None,
    ):
    ...
    def _read_application_logs(
        ...
        if isinstance(self.message_reader, PipesS3MessageReader) and isinstance(logs_uri, str):
            ...
            application_logs_prefix = os.path.join(
                prefix, f"{cluster_id}/{self.s3_application_logs_prefix or 'containers'}/"
            )
