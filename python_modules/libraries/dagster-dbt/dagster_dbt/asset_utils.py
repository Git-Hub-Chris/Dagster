...
def get_manifest_and_translator_from_dbt_assets(
    ...
    first_spec, *_ = [*dbt_assets_def.specs, *dbt_assets_def.check_specs]
    first_metadata = first_spec.metadata
    first_key = first_spec.key
    ...
    if manifest_wrapper is None:
        raise DagsterInvariantViolationError(
            f"Expected to find dbt manifest metadata on {first_key.to_user_string()},"
    ...
    if dagster_dbt_translator is None:
        raise DagsterInvariantViolationError(
            f"Expected to find dbt translator metadata on asset {first_key.to_user_string()},"
            " but did not. Did you pass in assets that weren't generated by @dbt_assets?"
        )
